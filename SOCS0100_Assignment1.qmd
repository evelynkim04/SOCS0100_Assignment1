...
The chosen dataset for this assignment was released by the WHO, and it displays the immunisation coverage among one year olds between 1974 and 2015 for diseases such as Tuberculosis (the data doesn't explicitly say tuberculosis immunisation coverage, because the disease is protected by the BCG vaccine), Hepatitis B, Diphtheria, Tetanus, Polio, and Measles in different regions of the world. It also includes data on the number of confirmed cases for Tetanus, Polio, Pertussis, Measles, and Diphtheria. Finally, the dataset showcases the estimated deaths due to tuberculosis excluding HIV.

We will begin by defining one of the key variables in the dataset: Immunisation coverage. It is the outcome of successful vaccination which accounts for full immunity to the disease. This dataset was chosen for this assessment as it is highly relevant in today’s social science studies. It wishes to determine the relationship between cases and immunisation coverage. This could be a potential indicator of the minimum immunisation coverage that is needed to reduce cases.
...
The dataset is thus useful when judging the overall quality of the country’s health care system, and whether it is consistent. Because some of these diseases require more than just one dose.

Let us look at the DTP3 vaccine (the third dose for prevention of Diphtheria and Tetanus) as an example. A low immunisation coverage and of Diphtheria and Tetanus may be a potential suggestion of a low consistency in the country’s healthcare system because people *need* to have 3 doses to have full immunity. There is thus high relevance of this dataset in social science, as it allows exploration of how socio-economic phenomenon influences the general state of a country’s healthcare [\[2\]](#_msocom_1) system.

Finally, the dataset contains quite a significant missing values. In general, there are no clear patterns in the NAs, but many are from countries with less developed healthcare systems, and are during the years before the 2000s. These descriptive findings are not unexpected, as it is likely that these less developed countries didn't have the means to record data given their social and economic situations.

## Part I-B

This section aims to carry out 3 data wrangling tasks using libraries such as Tidyverse packages, like dyplr. The first operation is renaming the dataset columns after having carried out all the necessary steps before hand (Removing everything in my environment and setting up working directory, downloading required packages such as pacman, and importing the "vaccine_2017.csv" data.)
...
The variable names are much cleaner now. However there are unnecessary columns at the far right (X, X.1, X.2...). The second data wrangling operation will thus be selecting variables and deleting these columns. The newly formed table will be called "data_clean".
...
In this section, 3 insightful data visualisations was carried out using libraries like ggplot2, derived from my refined "data_long" dataset.

Before commencing, the tidyverse package needs to be installed.

The new dataset that had been pivoted long still contained too many variables to visualise it. Critical engagement of Copilot became useful here, as it suggested to create average values of each measure and then group the countries by continent, as the dataframe contained over 100 different countries. The previous dataset was thus refined into a new one shown below:
...
It is now much easier to use this dataset for our first data visualisation, which is a heatmap. Critical engagement of AI made me derive to the conclusion that the very first data visualisation method should be easy to understand and provide a general overview of the trend in the data. A heatmap is a simple way to analyse where values are high or low, with the use of colour.
...
Unfortunately, the values for immunity and deaths from Tuberculosis per 100,000 aren't visible because they are significantly smaller than the other two measures. Copilot was used as a supporting tool, which suggested to apply a logscale to compress the large values, and make smaller ones more spread out.
...
Although the bar chart gives us insightful information on the exact values, the visual clutter of 4 different types of measures for each continent may not be the best way to present the data. This is why I decided to plot a dot plot for my final data visualisation, which is a much cleaner. The dot plot was separated into 4 different graphs (one for each measure) to avoid the visual clutter. The latter was suggested by AI tool, who was asked to suggest a solution for the visual clutter that had been a problem for the bar chart.
...
Automation allowed me to plot the graphs without having to write the codes each time.

Using these graphs, we may come down to several conclusions. Although cases are more or less similar in all continents (shown in plot 1), the average value of deaths from tuberculosis (plot 3) are significantly higher in Africa and Asia. This suggests that there is a socioeconomic factor that is involved. Many countries in Africa and Asia have less developed healthcare systems than for instance, Europe, which may be the reason for the higher number of deaths.

This trend is still observable in plot 4 which showcases the deaths of tuberculosis per 100,000 (hence the rate), although values are also high for Oceania as well for some further researchable reasons.

Finally, we may also conclude an overall negative relationship between cases and immunity, as shown in Figure 8. Relatively low immunisation coverage is always followed by a high number of cases.

## Part II-C

Critically engaging Copilot, and using it as a collaborative tool allowed for insightful perspectives, suggestions, and problem solving approaches. Utilising Copilot for functions that carried out specific tasks made a significant contribution to this assignment.

For instance, functions such as "str_to_lower()" used during data cleaning, or "facet_wrap()" for plotting the dot plot were all functions I wasn't familiar with that Copilot suggested to use in my line of code.

Furthermore, AI was used for explanation of certain functions, and correction of codes with errors - For example, use of AI allowed for identification of certain packages not having been downloaded, or of certain lines of code that were omitted.
...
The error message in the beginning was that certain column names had been omitted. Using AI as a support, I figured out that the country "Timor" could not be matched to the continent "Asia". Copilot suggested the use of the function "custom_match", which is what I decided to use to prevent all "Timor" data from becoming NAs.

One constraint however, was the suggestion of abundant ideas. For instance, when asking for advice on what type of graphs to plot, Copilot would suggest overly complicated, or irrelevant graphs.

Neverthelss, overall I believe use of AI helped in shaping the overall outcomes in this assessment, in the presentation of several of the plots, as well as in deciding the type of plots, in order to present data in the best possible way.

## Bibliography

[\[1\]](#_msoanchor_1) Fenner, F. (1987). "Veterinary virology", Orlando: Harcourt Brace Jovanovich.

[\[2\]](#_msoanchor_1) King's Global Health Institute. (2025), <https://www.kcl.ac.uk/kghi/research/health-and-social-science>
